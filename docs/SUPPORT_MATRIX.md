...
| Release package | Supported for aarch64 Linux | GitHub release workflow runs `./scripts/validate.sh` before release packaging, then `scripts/package-release.sh` builds the explicit `aarch64-unknown-linux-gnu` target and packages the binary, README, license, release notes, and checksums; `scripts/release-preflight.sh` checks local version/tree/tag readiness before the release tag is pushed |
