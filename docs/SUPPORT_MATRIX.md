...
| Release package | Supported for aarch64 Linux | GitHub release workflow runs `./scripts/release-preflight.sh --dry-run` with the tag or dispatch version before `./scripts/validate.sh`, then `scripts/package-release.sh` builds the explicit `aarch64-unknown-linux-gnu` target and packages the binary, README, docs, license, changelog, release notes, service installer, and checksums; `scripts/release-preflight.sh` also checks local version/tree/tag readiness before the release tag is pushed; `docs/RELEASE_PROCESS.md` documents the public-safe publish and post-publish installer-smoke sequence |
...
| Security headers | Supported for local API responses | All local API responses include `Cache-Control: no-store`, `X-Content-Type-Options: nosniff`, and `Referrer-Policy: no-referrer`; the local API validation smoke checks these headers on authenticated `/health` |
