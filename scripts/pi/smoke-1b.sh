...
usage() {
  cat <<'USAGE'
Usage: smoke-1b.sh [model.gguf] [chat|model|q8-chat|q8-model] [prompt] [max_tokens] [--q4|--q8] [--dry-run]
  ...
  case "$arg" in
    ...
    --q4)
      if [[ -n "$QUANT_MODEL" ]]; then
        echo "Only one 1B smoke quantization selector may be provided." >&2
        exit 2
      fi
      QUANT_MODEL="q4"
      ;;
...
if looks_like_non_gguf_model_path "${1:-}"; then
  echo "1B smoke model argument must be a .gguf path: $1" >&2
  exit 2
fi
...
smoke_status_json() {
  printf '{"target":"llama32-1b","status":"ok","model":%s,"selected_source":%s,"quantization":%s,"shape":"llama32_1b","shape_ready":true,"context_limit":%s,"smoke_prompt":%s,"smoke_kind":"%s","smoke_tokens":%s,"prefill_batch":%s}\n' \
    "$(json_string "$MODEL")" \
