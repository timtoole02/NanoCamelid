#!/usr/bin/env bash
# Start NanoCamelid's terminal chat against the default Pi-local 3B model.
set -euo pipefail
...
MODEL="${NANOCAMELID_MODEL_GGUF:-$WORKSPACE/models/Llama-3.2-3B-Instruct-Q4_0.gguf}"
...
if [[ ! -f "$MODEL" ]]; then
  ...
  echo "Set NANOCAMELID_MODEL_GGUF=/path/to/model.gguf or place the 3B Q4_0 GGUF at the default path." >&2
  exit 2
fi
...
if [[ "$SMOKE_ENABLED" != "0" ]]; then
  echo "Running $SMOKE_KIND smoke gate before launching 3B chat..."
