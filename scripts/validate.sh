...
Runs NanoCamelid's standard local validation gate:
  ...
  33a. ./scripts/pi/scarp-guard.sh --out /tmp/nanocamelid-scarp-validate --dry-run
  ...
  44. ./scripts/release-preflight.sh --dry-run
...
expect_file_contains "release workflow arm runner" "runs-on: ubuntu-24.04-arm" .github/workflows/release.yml
expect_file_contains "release workflow target install" "run: rustup target add aarch64-unknown-linux-gnu" .github/workflows/release.yml
expect_file_contains "release workflow package script" "run: ./scripts/package-release.sh" .github/workflows/release.yml
expect_file_contains "release workflow release notes body" "body_path: RELEASE_NOTES.md" .github/workflows/release.yml
expect_file_contains "release workflow checksum upload" "dist/SHA256SUMS" .github/workflows/release.yml
...
expect_failure_output "chat rejects unknown stable option" "unknown chat option" cargo run -- chat 1b --bogus --dry-run
...
echo "==> Checking SCARP guard dry run..."
./scripts/pi/scarp-guard.sh --out /tmp/nanocamelid-scarp-validate --dry-run
expect_output "scarp-guard lists the untied control row" "llama3-8b-q4_k_m" ./scripts/pi/scarp-guard.sh --list
expect_output "scarp-guard dry run labels the untied control" "head_kind=untied" ./scripts/pi/scarp-guard.sh --out /tmp/nanocamelid-scarp-validate --dry-run
expect_failure_output "scarp-guard requires an output dir" "--out is required" ./scripts/pi/scarp-guard.sh --dry-run
expect_failure_output "scarp-guard rejects an unknown row" "Unknown row id: nope" ./scripts/pi/scarp-guard.sh --out /tmp/nanocamelid-scarp-validate --rows nope --dry-run
expect_failure_output "scarp-guard rejects an unknown prompt" "Unknown prompt id: medium" ./scripts/pi/scarp-guard.sh --out /tmp/nanocamelid-scarp-validate --prompts medium --dry-run
expect_failure_output "scarp-guard rejects an invalid token count" "--tokens must be a positive integer: bad" ./scripts/pi/scarp-guard.sh --out /tmp/nanocamelid-scarp-validate --tokens bad --dry-run
...
expect_output "remote_build help documents remote target dir" "Optional absolute Cargo target dir" bash -c './scripts/remote_build.sh --help 2>&1'
...
expect_failure_output "remote_build rejects relative workspace" "NANOCAMELID_REMOTE_WORKSPACE must be an absolute path" env NANOCAMELID_REMOTE_WORKSPACE=nanocamelid ./scripts/remote_build.sh "<redacted-pi-host>" --dry-run
expect_failure_output "remote_build rejects relative target dir" "NANOCAMELID_REMOTE_TARGET_DIR must be an absolute path" env NANOCAMELID_REMOTE_TARGET_DIR=target ./scripts/remote_build.sh "<redacted-pi-host>" --dry-run
expect_failure_output "remote_build rejects repo-local target dir" "Refusing to use a remote repo-local Cargo target dir" env NANOCAMELID_REMOTE_TARGET_DIR=/mnt/nanocamelid/src/NanoCamelid/target ./scripts/remote_build.sh "<redacted-pi-host>" --dry-run
...
expect_failure "remote_build disabled smoke invalid context limit" env NANOCAMELID_REMOTE_SMOKE=0 NANOCAMELID_REMOTE_CONTEXT_LIMIT=bad ./scripts/remote_build.sh "<redacted-pi-host>" --dry-run
expect_failure "remote_build disabled smoke invalid prefill batch" env NANOCAMELID_REMOTE_SMOKE=0 NANOCAMELID_REMOTE_PREFILL_BATCH=bad ./scripts/remote_build.sh "<redacted-pi-host>" --dry-run
expect_failure "remote_build disabled smoke invalid context caps" env NANOCAMELID_REMOTE_SMOKE=0 NANOCAMELID_REMOTE_CONTEXT_PACKS=512,bad ./scripts/remote_build.sh "<redacted-pi-host>" --dry-run
...
echo "==> Checking release publication preflight dry run..."
...
expect_output "release preflight installer default version" "installer_default: v0.1.0" ./scripts/release-preflight.sh --dry-run
expect_output "release preflight installer target" "installer_target: aarch64-unknown-linux-gnu" ./scripts/release-preflight.sh --dry-run
